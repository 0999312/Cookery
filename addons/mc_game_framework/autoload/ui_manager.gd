...
func _show_background_dimmer(layer: int) -> void:
	if _dimmers.has(layer):
		...
		_dimmers[layer].mouse_filter = Control.MOUSE_FILTER_STOP
		return
	_ensure_layer(layer)
	var layer_node: CanvasLayer = _layer_nodes[layer]
...
func _hide_background_dimmer(layer: int) -> void:
	if _dimmers.has(layer):
		...
		_dimmers[layer].mouse_filter = Control.MOUSE_FILTER_IGNORE

## 隐藏所有背景遮罩并清空缓存（用于场景切换前的完全清理）
func clear_for_scene_change() -> void:
	# 隐藏所有遮罩
	for layer in _dimmers.keys():
		_dimmers[layer].visible = false
		_dimmers[layer].mouse_filter = Control.MOUSE_FILTER_IGNORE
	# 清空面板缓存
	for id_str in _cached_panels.keys():
		var panel: UIPanel = _cached_panels[id_str]
		if is_instance_valid(panel):
			panel._on_destroy()
			panel.queue_free()
	_cached_panels.clear()
	_cache_order.clear()
	# 清空弹窗队列
	_popup_queue.clear()
	_current_queued_popup = null
