...
func set_diagnostics(p_diagnostics: Array) -> void:
	_diagnostics = p_diagnostics

func append_diagnostics(p_diagnostics: Array) -> void:
	_diagnostics.append_array(p_diagnostics)
...
func map(transform: Callable) -> DataResult:
	...
	new_result.set_diagnostics(_diagnostics.duplicate())
...
func flat_map(transform: Callable) -> DataResult:
	...
	merged.append_diagnostics(inner._diagnostics)
	# 如果原结果是 PARTIAL，降级
