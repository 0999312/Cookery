...
##
## 注意：ResourceLocation 继承自 RefCounted，Godot Dictionary 使用对象引用比较
## 而非 equals()。请始终使用 .to_string() 作为 Dictionary key，勿直接用 ResourceLocation。
extends RefCounted
...
## 缓存的 RegEx 实例（避免每次校验都 new RegEx + compile）
static var _namespace_regex: RegEx
static var _path_regex: RegEx
...
static func from_string(location_str: String) -> ResourceLocation:
	if location_str.is_empty():
		push_warning("ResourceLocation.from_string: empty string")
		return null
	var parts = location_str.split(":", true, 1)
	if parts.size() != 2:
		push_warning("Invalid ResourceLocation format: " + location_str)
		return null
	if parts[0].is_empty() or parts[1].is_empty():
		push_warning("ResourceLocation namespace and id must not be empty: " + location_str)
		return null
...
static func validate_namespace(ns: String) -> DataResult:
	...
	if not _namespace_regex:
		_namespace_regex = RegEx.new()
		_namespace_regex.compile(NAMESPACE_PATTERN)
	if not _namespace_regex.search(ns):
...
static func validate_path(path: String) -> DataResult:
	...
	if not _path_regex:
		_path_regex = RegEx.new()
		_path_regex.compile(PATH_PATTERN)
	if not _path_regex.search(path):
		return DataResult.error(
